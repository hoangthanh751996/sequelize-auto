...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  const newTables = tables.map(t => {
    ...
    arr = arr.map(item => {
      ...
      let last = item.substring(1, t.length);
      first = first.toUpperCase();
      return first + last;
    });
    return arr.join("");
  }).sort();
  fs.writeFileSync(path.join(self.options.directory, "models.json"), JSON.stringify({models: newTables}), {encoding:'utf8',flag:'w'});
  ...
  function createFile(table, _callback) {
    let fileName = table;
    if (self.options.camelCaseForFileName) {
      let arr = table.split("_");
      arr = arr.map(t => {
        let first = t.substring(0, 1);
        let last = t.substring(1, t.length);
        first = first.toUpperCase();
        return first + last;
      });
      fileName = arr.join("");
    }
    fs.writeFile(path.resolve(path.join(self.options.directory, fileName + (self.options.typescript ? '.ts' : '.js'))), attributes[table], {encoding:'utf8',flag:'w'}, _callback);
  }
};
